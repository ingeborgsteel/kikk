...
- **Query cache persistence**: `src/react-app/main.tsx` wraps `QueryClient` in `PersistQueryClientProvider` with `createSyncStoragePersister` (localStorage key: `kikk-query-cache`); `gcTime` 7 days
