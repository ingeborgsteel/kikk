...
export function ObservationsProvider({ children }: { children: ReactNode }) {
  ...
  const { data: remoteObservations = [] } = useFetchObservations();
  ...
  const observations = useMemo(() => {
    const remoteIds = new Set(remoteObservations.map((o) => o.id));
    const pending = offlineQueue.filter((o) => !remoteIds.has(o.id));
    return [...pending, ...remoteObservations];
  }, [remoteObservations, offlineQueue]);
  ...
  }, [isOnline, offlineQueue.length, drainQueue]);
  ...
  const updateObservation = useCallback(
    (updatedObservation: Observation) => {
      const isQueued = offlineQueue.some((o) => o.id === updatedObservation.id);
      if (isQueued) {
